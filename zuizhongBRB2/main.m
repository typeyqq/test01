...
for m=1:1  %152=25X5+25+2
    ...
    plot(k,AllData(k,3),'b',k,Z(k),'r',k,Y(k),'g');
    %plot(k,AllData(k,3),'b',k,Y(k),'g');
    %plot(k,AllData(k,3),'b',k,Z(k),'r');
    legend('实际值', '优化BRB','初始BRB');
    %绘制点图
    %粗为100，细50，点为实际
end
yMSE(m) = fun_test(ybest)
