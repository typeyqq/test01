function bestx=test_qqq(x,G,Aeq,beq,ub,lb)
...
while counteval < stopeval
    ...
    for k=1:lambda
        ...
        for i=1:a
            ...
            norm_BB = norm(arx(ini(i):(ini(i)+num(i)-1),k),3);
            arx(ini(i):(ini(i)+num(i)-1),k) = arx(ini(i):(ini(i)+num(i)-1),k) / norm_BB;
            arx(ini(i):(ini(i)+num(i)-1),k)=arx(ini(i):(ini(i)+num(i)-1),k)./sum(arx(ini(i):(ini(i)+num(i)-1),k));
            
            % 初始化矩阵BB
            %{
            arx(ini(i):(ini(i)+num(i)-1),k) = rand(4, 1); % 随机初始化BB

            % L2范数归一化
            norm_BB = norm(arx(ini(i):(ini(i)+num(i)-1),k), 1);
            arx(ini(i):(ini(i)+num(i)-1),k) = arx(ini(i):(ini(i)+num(i)-1),k) / norm_BB;
