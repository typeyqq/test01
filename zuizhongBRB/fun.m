...
for n = 1:T  %导入测试集            %%至95    %所以每个测试的数据相互之间是独立的
    ...
    l2 = TrainData (n,2);
    y1=[4.379 2.441 2.223 1.636 1];    % x1的参考值   同时确保最大值最小值把数据包裹住 输入的参考值
    y2=[3.635 1.487 1.196 1.037 0.999];     % x2的参考值
    T1=length(y1);
    T2=length(y2);
    In=zeros(L,M);
    ...
    Doutput = [2.947 1.941 1.566 1.316 0.999];   %此处是输出的参考值
    Sum1 = sum(  beta1' );
    for j = 1:N
        temp1(j) = 1;
        for k = 1:L
