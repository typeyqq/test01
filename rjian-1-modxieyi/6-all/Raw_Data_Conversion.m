...
for i = 1:n(1)%一行一行的处理
    ...
    output5 = zeros(1,5);%用来存储第i行的第五个属性数据转化为的置信度
    for j = 1:5 %在data.txt中有三列
        switch j
            case 1
                output1 = Belief_Degree_Ascending(y,data(i,j)); %原始数据转置信度               
            case 2
                output2 = Belief_Degree_Ascending(y,data(i,j));             
            case 3
                output3 = Belief_Degree_Ascending(y,data(i,j));     
            case 4
                output4 = Belief_Degree_Ascending(y,data(i,j));
            case 5
                output5 = Belief_Degree_Ascending(y,data(i,j));
        end
    end
